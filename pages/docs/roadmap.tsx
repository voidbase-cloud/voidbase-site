  ...
  return (
    <article className="why">
      ...
      <Item
        ...
        now="Sign in with Cloudflare, name an instance, and your browser creates it in your own account with its database, storage and domain: voidbase.cloud keeps your sign-in, your sealed Cloudflare and GitHub tokens and the rows, and passes your calls through; the work is the page's, with voidbase's own code. From its page you upgrade it to the active release, wire a repository to it (created from a template or linked), and sign in to the instance itself to install, update and remove plugins through its own installer: on a project instance a change is a commit its repository's build deploys. Logs, metrics, domains, secrets and backups still live in Cloudflare's dashboard and the instance's own panel."
