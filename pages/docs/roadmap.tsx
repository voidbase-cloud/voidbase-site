  ...
  return (
    <article className="why">
      ...
      <Item
        ...
        now="voidbase update covers the executable, a global install and a project's dependency, and voidbase cloud now upgrades an instance in place from its page: the same Worker, its database, files, domains and superuser kept, the migrations of the new release run on the next boot. An instance with plugins is rebuilt on the new base by the builder rather than re-provisioned bare, so it keeps them. Not rolled per instance by us, and not reversible yet: an upgrade you cannot undo is one people press carefully."
      ...
      <Item
        ...
        now="Sign in with Cloudflare, name an instance, and your browser creates it in your own account with its database, storage and domain: voidbase.cloud keeps your sign-in, your sealed Cloudflare and GitHub tokens and the rows, and passes your calls through; the work is the page's, with voidbase's own code. From its page you upgrade it to the active release, wire a repository to it (created from a template or linked), and sign in to the instance itself to install, update and remove plugins through its own installer: on a project instance a change is a commit its repository's build deploys. From the same card: the instance's logs with a filter, its superusers, its custom domains and its Worker secrets, each a call the page makes itself. Metrics and backups still live in Cloudflare's dashboard and the instance's own panel."
      ...
      <Item
        ...
        now="The body limit and the rate limit are a plugin providing hardening@1, held in place by one slot in the request chain; remove it and there are no limits. The headers on every response (X-Content-Type-Options nosniff, X-Frame-Options SAMEORIGIN, X-Xss-Protection, Cross-Origin-Opener-Policy same-origin) and the strict Content-Security-Policy on file responses are still the server's own. CORS is wide open, origin *, with Authorization allowed. No CSRF anywhere and no SameSite handling."
