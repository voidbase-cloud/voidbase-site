  ...
  return (
    <article className="why">
      ...
      <Item
        ...
        status="shipped"
        left="The Durable Object database becoming the default rather than a knob, once more instances have run on it."
        ...
        now="Shipped by 0.9.0-beta.41. VOIDBASE_DATABASE=durable puts an instance's data in its own SQLite-backed Durable Object instead of D1: the deploy binds the object and skips D1, the same interface is served over it, and a batch is one real transaction, so a failing second statement leaves nothing of the first. A hook can open one too: $app.runInTransaction buffers its writes and sends them as a single batch, rolling everything back on a throw, and $app.transactionsAreReal says whether the instance can do that. The limit is measured rather than assumed: a Durable Object cannot hold a transaction open across a call back into the Worker, so a hook cannot read a table it has already written inside the transaction, and trying throws rather than answering something stale. The 100-parameter and 100-column ceilings are SQLite's on Cloudflare and stay."
      ...
      <Item
        ...
        now="Shipped in @voidbase-cloud/sdk, in two halves. 0.1.0 brought the queue: a mutation the network refuses, or one attempted while the browser says it is offline, is queued in localStorage (or a store you hand in), answered optimistically with an id chosen up front, and replayed in order when the connection is back; a 4xx on replay drops it and says so. 0.6.0 brought the reads: a cached collection answers getOne and getList from the store when the network refuses, or straight away while it revalidates behind you, written through on every success, kept fresh by realtime and by the queue's own optimistic answers, and capped per collection. It is a cache for rendering rather than a local database, and the docs say so. The adapter's pwa option and the SDK's pwa helper put the service worker around it."
      ...
      <Item
        ...
        now="Shipped in two steps. voidbase types writes the Collections map from the instance's own OpenAPI document, and @voidbase-cloud/sdk, voidbase's fork of the PocketBase SDK, takes it as new VoidBase&lt;Collections&gt;(url), so a renamed field is a compile error at the call. --watch keeps the file in step: it polls the instance and rewrites only when the generated output actually changes, naming what moved. The same client has the plugin surface: client.use(plugin), hook lists on the request path, and six shipped plugins that use it (offline, pwa, editable, ai, payments, seo)."
      ...
      <Item
        ...
        status="shipped"
        left="Rolling an upgrade per instance from our side rather than yours."
        ...
        now="voidbase update covers the executable, a global install and a project's dependency, and voidbase cloud now upgrades an instance in place from its page: the same Worker, its database, files, domains and superuser kept, the migrations of the new release run on the next boot. An instance with plugins keeps them: they live in its own repository, so the next push after the upgrade deploys them on the new base. An upgrade is reversible for seven days: the page records the release it left and offers the way back, which re-deploys the code and clears the record. It does not undo the migrations the newer release ran, and the page says so where you press it."
      ...
      <Item
        ...
        now="Shipped in full by 0.9.0-beta.38. The body limit and the rate limit are a plugin providing hardening@1, and so is the response policy: the headers on every response, a strict policy on files, and knobs that are off unless set (named CORS origins instead of the wildcard, HSTS, Referrer-Policy, Permissions-Policy, the cross-origin trio, a Content-Security-Policy for every response and VOIDBASE_CSP_ROUTES for the paths that need their own). Cross-site writes have two defences: the origin rule, which refuses a state-changing cookie request from an origin not on the named list, and VOIDBASE_CSRF=double-submit, where GET /api/csrf sets a cookie and hands back a token that a cookie-authenticated write must echo in a header; a request authenticated by a bearer token is exempt, because a browser never attaches one on its own. voidbase check --security reads a running instance from outside and reports pass, warn or fail per line with the knob that fixes it."
      ...
      <Item
        ...
        status="shipped"
        left="The panel offering the archive kind and the verification, which the cloud dashboard's card already does."
        from="The built-in backup is a zip in the same account as the thing it is backing up."
        now="Shipped in 0.9.0-beta.31 inside the backups plugin, and streaming since 0.9.0-beta.36. A full archive holds every table, every file, the settings with secrets left out and the schema, with a manifest that names each entry's hash; a data archive holds the non-system collections' rows and files for moving between environments. Each archive is read back and verified after it is written and on demand, restore refuses an archive from a newer voidbase and never invents a collection unasked, the scheduled backup keeps VOIDBASE_BACKUP_KEEP of them, and VOIDBASE_BACKUP_S3_* copies every archive to a bucket outside the account with a hand-signed SigV4 upload. Restore reads the archive as one stream and loads it in batches, so an archive far larger than the isolate's memory restores: 306 MB of rows restored in about 40 MB of resident memory in the package's own test."
      ...
      <Item
        ...
        now="Shipped in full by 0.9.0-beta.36 as the seo plugin. robots.txt, a sitemap from the public records VOIDBASE_SITEMAP names (with hreflang alternates once locales are declared) and llms.txt came first, each yielding to a real file and reaching a deployed Worker through the _redirects rules the adapter writes. Then GET /api/seo/meta resolves a page path back to its record and answers the canonical URL, title, description, JSON-LD from the schema.org mapping VOIDBASE_SEO declares, OpenGraph and Twitter tags and a ready head fragment; every answer carries an ETag and the Worker version so a cache revalidates cleanly across a deploy. Share cards render on request: SVG by default, and PNG where VOIDBASE_SEO_PNG is set, which carries a WebAssembly rasteriser and a subset font into the Worker (about a megabyte compressed, so it is a choice rather than a default). The client side is @voidbase-cloud/sdk/seo."
      ...
      <Item
        ...
        now="Shipped in both halves. Content: VOIDBASE_TRANSLATABLE names the fields per collection and VOIDBASE_LOCALES the locales in fallback order, translations live in a collection the plugin owns, and the records API answers list and view, expands included, in the locale the request asks for, with Content-Language and a per-record note of which fields were swapped, in one batched lookup. Interface strings: voidbase i18n extract scans the project for its translation calls and writes a flat catalogue per locale plus a declaration of the keys in use, so naming a key nothing calls stops compiling, and --check fails a build with anything untranslated; the runtime is i18n() from @voidbase-cloud/sdk/i18n, typed from those keys, which also sends Accept-Language so the content answers in the same language. A locale in the route is an adapter option, and the cloud dashboard has a panel showing what is untranslated per collection and locale, where a few strings can be filled in place."
      ...
      <Item
        ...
        left="Binding a cancellation to the payment the order's own checkout started: at Lemon Squeezy, where the order reference is the buyer's to set, a declined purchase of the same variant can still cancel another of that payer's pending orders."
      />
