...
export default function DocsRoadmap() {
  return (
    <article className="why">
      ...
      <Item
        title="Auth as a plugin surface, with PocketBase's rebuilt on Better Auth"
        from="Void ships Better Auth. voidbase ships PocketBase's. Bolting a second auth system on beside the first would leave two session models, two sets of tables and a permanent argument about which one owns a user."
        now="Authentication is built into the server: about twelve hundred lines across auth, its flows, OTP, MFA, WebAuthn, JWT and OAuth2, owning _superusers, _externalAuths, _authOrigins, _otps and _mfas, issuing PocketBase-shaped HS256 tokens signed with each record's own tokenKey. It works and it is exactly right for compatibility. It is also welded in, so there is no way to have different auth without editing the server."
        plan="Take auth out of the core and make it a surface plugins fill. Better Auth becomes the engine, and PocketBase's auth is rebuilt on top of it as a plugin that ships with voidbase and is on by default, so nothing changes for anyone who does not go looking. Better Auth's plugin system already has the parts this needs: plugins add their own endpoints, bring their own schema, hook the request and response lifecycle, and can take over token signing, which is what makes a foreign token format possible at all. The compatibility plugin keeps the tables it owns, maps auth-with-password, auth-with-oauth2, auth-with-otp, auth-refresh and the request and confirm flows onto Better Auth's, and keeps issuing the tokens every existing client already parses. What that buys is the thing worth having: an instance can then run PocketBase's auth, or Better Auth's own surface with organisations, passkeys and two-factor, or both, and somebody else can publish an auth plugin for an identity provider we have never heard of without us being involved."
        size="The largest thing on this page, and it lands after the plugin loader, because a loader that cannot carry auth is not a loader worth having. The part that decides whether it is possible at all: PocketBase signs each token with the user's own key, so changing a password invalidates that user's tokens with no session table anywhere. Better Auth is session-first. Reproducing that exactly, rather than approximately, is the question this stands or falls on, and we do not know the answer yet."
      />
