  ...
  return (
    <article className="why">
      ...
      <Item
        ...
        now="The first half shipped in 0.9.0-beta.29 as the seo plugin: robots.txt, a sitemap generated from the public records VOIDBASE_SITEMAP names (a collection and a path template per line, with lastmod from updated), and llms.txt from the public collections, every one of them yielding to a real file in pb_public. On Cloudflare the adapter writes the _redirects rules that carry the three paths to the Worker, so the asset layer stays in front of everything else. JSON-LD, OpenGraph and Twitter tags, canonical URLs and share images are still to come."
