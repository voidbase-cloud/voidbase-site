...
import { connectionFor, dispatchBuilder, instanceJSON, isAdmin, pluginsOf, releaseSource, requireAuth, userId } from "@/shared";
...
export const POST = defineHandler(requireAuth("users"), async (c) => {
  ...
  if (pluginsOf(row).length) {
    ...
    const started = (await dispatchBuilder(`upgrade of ${name}`)) === "started";
    return { instance: instanceJSON(row, auth), upgraded: false, queued: true, from, to, message: `Queued a build on ${to} with this instance's plugins${started ? " and started the builder" : ""}; it takes minutes.` };
  }
